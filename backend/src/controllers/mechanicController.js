...
exports.createOrUpdateProfile = async (req, res) => {
  try {
    const allowed = ['name', 'specialization', 'equipmentTypes', 'experience', 'description', 'hourlyRate', 'city', 'state', 'latitude', 'longitude', 'certifications', 'isAvailable', 'profileImage', 'mobileService'];
    const data = {};
...
exports.getMyProfile = async (req, res) => {
  try {
    const profile = await MechanicProfile.findOne({ where: { userId: req.userId } });
    res.json({ profile });
  } catch (err) { res.status(500).json({ message: err.message }); }
};
