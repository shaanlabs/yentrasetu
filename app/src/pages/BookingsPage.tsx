              ...
              <div key={b.id} className="bg-white rounded-xl shadow-sm border border-[#E9E3DA] p-5">
                ...
                {b.status === 'pending' && role === 'owner' && (
                  <div className="flex gap-2 pt-3 border-t border-[#E9E3DA]">
                    <button onClick={() => handleAction(b.id, 'confirmed')} className="text-xs px-3 py-2 bg-green-50 text-green-700 rounded-lg border border-green-200 flex items-center gap-1 min-h-[36px]"><CheckCircle size={12} /> Confirm</button>
                    <button onClick={() => handleAction(b.id, 'cancelled')} className="text-xs px-3 py-2 bg-red-50 text-red-700 rounded-lg border border-red-200 flex items-center gap-1 min-h-[36px]"><XCircle size={12} /> Reject</button>
                  </div>
                )}
                {b.status === 'pending' && role === 'renter' && (
                  <div className="flex gap-2 pt-3 border-t border-[#E9E3DA]">
                    <button onClick={() => handleAction(b.id, 'cancelled')} className="text-xs px-3 py-2 bg-red-50 text-red-700 rounded-lg border border-red-200 flex items-center gap-1 min-h-[36px]"><XCircle size={12} /> Cancel</button>
                  </div>
                )}
              </div>
            ))}
