...
export default function SaleDetailPage() {
  ...
  const [emiDown, setEmiDown] = useState(20); // 20%
  const [emiRate, setEmiRate] = useState(12); // 12%
  const [emiTenure, setEmiTenure] = useState(3); // 3 years
  ...
  const seoSchema = listing ? {
    ...
    "description": listing.description || `${listing.year ? listing.year + ' ' : ''}${listing.make} ${listing.model} ${listing.category} available for sale in ${listing.city || 'India'}.`,
    "sku": actualId,
    "brand": {
      "@type": "Brand",
      "name": listing.make
  ...
  useSEO({
    ...
    description: listing?.description?.substring(0, 160) || `Buy ${listing?.make} ${listing?.model} on YantraSetu.`,
  ...
  const handleContactSeller = async () => {
    if (!isAuthenticated) {
      navigate('/login', { state: { returnTo: `/sale/${id}` } });
      return;
    }
    setContactLoading(true);
    try {
      const data = await chatsApi.startOrGet(listing!.owner!.id as string, 'sale', listing!.id);
  ...
  const handleEnquirySubmit = async () => {
    if (!enquiryForm.name || !enquiryForm.phone || !enquiryForm.city) {
      setEnquiryMsg('Please fill out all required fields.');
      return;
    }
    setEnquiryLoading(true);
    setEnquiryMsg('');
    try {
      await machineryApi.submitSaleEnquiry(actualId!, enquiryForm);
      setEnquiryMsg('Success! The seller will contact you shortly.');
      setTimeout(() => setShowEnquiryModal(false), 2000);
    } catch (err: any) {
      setEnquiryMsg(err.message || 'Failed to submit enquiry.');
    } finally {
  ...
  const handleWhatsApp = () => {
    if (!listing) return;
    const text = `Hi, I'm interested in your ${listing.make} ${listing.model} (${formatPrice(listing.price)}) listed on YantraSetu. ${window.location.href}`;
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `Check out this ${listing?.make} ${listing?.model} — ${formatPrice(listing?.price || 0)}`,
      url: window.location.href,
    };
    if (navigator.share) {
  ...
  return (
    <PageShell
      ...
      backTo="/browse"
      backLabel="Browse"
      seoTitle={`${listing.make} ${listing.model} - ${listing.city || 'India'} | YantraSetu`}
      seoDescription={`Buy ${listing.condition || 'Used'} ${listing.make} ${listing.model} for ${formatPrice(listing.price)} in ${listing.city || 'India'}. Verified seller on YantraSetu.`}
    >
      <div className={`grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 ${!isOwner ? 'pb-44 sm:pb-0' : ''}`}>
        ...
        <div className="lg:col-span-3">
          ...
          <div className="relative bg-white rounded-xl overflow-hidden shadow-sm border border-[#EDE8E0] mb-5">
            ...
            <div className="absolute top-4 left-4 flex gap-2">
              <Badge className="bg-[#FF6A00] hover:bg-[#e55f00] font-mono tracking-wider">
                FOR SALE
              </Badge>
              {listing.isVerified && (
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Price card */}
          <Card className="border-[#EDE8E0] shadow-sm lg:sticky lg:top-24 z-20 bg-white">
            <CardContent className="p-5 sm:p-6">
                ...
                <div className="space-y-3">
                  <div className="flex flex-col gap-2">
                    <Button
                      ...
                      className="w-full h-12 bg-[#FF6A00] hover:bg-[#e55f00] text-white text-[15px] font-bold shadow-sm"
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex-1 h-12 border-[#25D366] text-[#25D366] hover:bg-[#25D366] hover:text-white transition-colors font-bold"
                      >
                        WhatsApp
                      </Button>
                      <Button
              ...
              {listing.listingType === 'sale' && (
                <Button
                  ...
                  className="w-full mt-5 h-11 border-2 border-[#FF6A00] text-[#FF6A00] bg-[#FF6A00]/5 hover:bg-[#FF6A00] hover:text-white transition-all font-semibold"
                  style={{ fontFamily: 'Sora, sans-serif' }}
                >
                  <Calculator className="mr-2 h-4 w-4" /> {showEmi ? 'Hide EMI Calculator' : 'Calculate EMI'}
                </Button>
              )}
            </CardContent>
          </Card>
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <Card className="border-[#EDE8E0] shadow-sm">
                  ...
                  <CardContent className="space-y-5">
                    <div>
                      <div className="flex justify-between text-xs text-[#6F757C] mb-2 font-medium" style={{ fontFamily: 'DM Sans, sans-serif' }}>
                        <span>Down Payment</span><span className="font-bold text-[#101214]">{emiDown}% — {formatPrice(listing.price * emiDown / 100)}</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs text-[#6F757C] mb-2 font-medium" style={{ fontFamily: 'DM Sans, sans-serif' }}>
                        <span>Tenure</span><span className="font-bold text-[#101214]">{emiTenure} years</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs text-[#6F757C] mb-2 font-medium" style={{ fontFamily: 'DM Sans, sans-serif' }}>
                        <span>Interest Rate</span><span className="font-bold text-[#101214]">{emiRate}%</span>
                      </div>
                      <input type="range" min={8} max={16} step={0.5} value={emiRate} onChange={e => setEmiRate(Number(e.target.value))}
                        className="w-full h-2 bg-[#EDE8E0] rounded-lg appearance-none cursor-pointer accent-[#FF6A00]" />
                    </div>
                    <div className="bg-[#FF6A00]/5 rounded-xl p-5 text-center border border-[#FF6A00]/20">
                      <p className="text-xs font-semibold text-[#6F757C] mb-1 uppercase tracking-wider" style={{ fontFamily: 'IBM Plex Mono, monospace' }}>Estimated Monthly EMI</p>
                      <p className="text-3xl font-bold text-[#FF6A00]" style={{ fontFamily: 'Sora, sans-serif' }}>
                        {formatPrice(Math.round(emiMonthly))}
                      </p>
                      <p className="text-[11px] font-medium text-[#6F757C] mt-2" style={{ fontFamily: 'DM Sans, sans-serif' }}>Loan: {formatPrice(Math.round(emiLoanAmount))} · {emiMonths} months</p>
                    </div>
                    <Button
                      variant="link"
                    ...
                    >
                      <ExternalLink className="mr-2 h-4 w-4" /> Apply for Loan
                    </Button>
                  </CardContent>
                </Card>
              </motion.div>
